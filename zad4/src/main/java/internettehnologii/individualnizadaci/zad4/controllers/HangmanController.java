...
public class HangmanController {
    ...
    public String hangmanView(Model model, HttpSession session,
        ...
        if(optionRadio != null){
            switch (optionRadio) {
                case "CGI" -> response = clientService.sendPostRequest("http://localhost:8081/hangman.cgi",letter);
                case "Java" -> response = clientService.sendPostRequest("http://localhost:8082/hangman",letter);
                case "DotNet" -> response = clientService.sendPostRequest("http://localhost:8083/hangman",letter);
